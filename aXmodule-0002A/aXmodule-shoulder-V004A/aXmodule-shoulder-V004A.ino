...
//  1
const int monoPattern0frames = 29;
byte monoPattern0[monoPattern0frames][thisPartNoLEDs];

//  2
const int monoPattern1frames = 25;
byte monoPattern1[monoPattern1frames][thisPartNoLEDs];

//  3
const int monoPattern2frames = 19;
byte monoPattern2[monoPattern2frames][thisPartNoLEDs];

//  4
const int monoPattern3frames = 22;
byte monoPattern3[monoPattern3frames][thisPartNoLEDs];

//  5
const int monoPattern4frames = 95;
byte monoPattern4[monoPattern4frames][thisPartNoLEDs];

//  6
const int monoPattern5frames = 32;
byte monoPattern5[monoPattern5frames][thisPartNoLEDs];
        ...
        while (workingFile.available() != 0) {  
          ...
          if(currentPatternNo < noMonoPatterns) {
            ...
            if(currentPatternNo == 0) {
              ...
              monoPattern3[frameLine][thisByte] = thisColor;
            } else if(currentPatternNo == 4) {
              monoPattern4[frameLine][thisByte] = thisColor;
            } else if(currentPatternNo == 5) {
              monoPattern5[frameLine][thisByte] = thisColor;
            };
...
//        feedback += currentFrameNo;
        if(!patternPause) {
          if (currentPatternNo == 0) {
              ...
              autoPattern(monoPattern3frames, 70, 5, currentColour, true, false);
          } else if (currentPatternNo == 5) {
              autoPattern(monoPattern4frames, 15, 5, currentColour, true, false);
          } else if (currentPatternNo == 6) {
              autoPattern(monoPattern5frames, 15, 5, currentColour, true, false);
  
              
          } else if (currentPatternNo == 10) {
              autoPattern(colorPattern0frames, 0, 10, currentColour, true, false);
          } else if (currentPatternNo == 11) {
              autoPattern(colorPattern1frames, 0, 10, currentColour, true, false);
          } else if (currentPatternNo == 12) {
...
void autoPattern(int numberFrames, int delayWait, int noCycles, int colourType, bool patternChanger, bool reversible) {
  ...
  while (currentFrameNo < numberFrames) {
        ...
        if (currentPatternNo == 0) {
            ...
            runPattern(monoPattern3[currentFrameNo]);
        } else if (currentPatternNo == 5) {
            runPattern(monoPattern4[currentFrameNo]);
        } else if (currentPatternNo == 6) {
            runPattern(monoPattern5[currentFrameNo]);
